...
在接受到用户命令时，**优先检查是否有可以处理用户诉求的skill**：
- 如果用户的请求匹配某个skill的能力范围（如 `/revealjs`、`/pptx`、`/git-commit` 等），应优先调用对应skill
- Skill作为专业领域的首选处理方式，只有当没有可用skill时才直接处理
- 调用方式：在Skill工具中设置 `skill` 为目标skill名称
...
发现代码与文档不一致时，必须记录到 `docs/tech-debt-todo.md`：
...
|---|------|---------|---------|----------|------|
| TD-XXX | 描述 | 文档怎么说 | 代码实际怎样 | YYYY/MM/DD | 待处理 |
```

### 严重程度
| 等级 | 含义 | 处理时限 |
|------|------|----------|
| 🔴 严重 | 功能完全缺失或核心架构不符 | 立即处理 |
| 🟡 中等 | 功能部分实现或实现不符文档 | 下一迭代 |
| 🟢 轻微 | 代码质量问题或小缺失 | 后续优化 |

### 决策事项
每个严重问题必须附带决策选项：需要产品确认的列在"决策事项"中，需要技术方案的在"修复计划"中说明。

### 修复完成后
在 `docs/tech-debt-todo.md` 的"处理记录"表格中追加一行，说明修复日期、操作和结果。